...
| ✅ **v90 SHIPPED** | **Force-out RBI awareness** — Contextual rule hint under the RBI buttons in the play-log modal, swapping based on result code. Practical interpretation per manager: any time a kid causes a run to score, give them the RBI; only DP/GIDP excludes (and that's an opt-in result the manager taps). The technical LL 9.04 "run scored solely due to fielding error" exception is **NOT** surfaced — too hard to track in real-time per manager 5/3 ("would love to get to the point of being able to actually do that but it is so hard to get that entered in time"). |
| 💡 _someday_ | **Error-attributed run detection** — When a fielding error is logged on a play that also produces a run, optionally flag "this run scored due to the error — exclude RBI?" per LL 9.04. Requires linking the standalone-error / in-AB-error record to the run-scoring event in time. Manager 5/3: "I would love to get to the point of being able to actually do that." Deferred until in-game error logging is fast enough to do live. |
