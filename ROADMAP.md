...
| 💡 | **Pitcher rotation tiers** — Manager pairs pitchers in tiers (Brody/Ezra are tier-1; Miles/Ethan are tier-2; everyone else is "developing"). Capture as `roster[i].pitcherTier` (1/2/3/null). Pre-game pitcher-planning view suggests "tier-1 + tier-2 + developing" rotations and warns when one is missing. | Manager feedback v88: "We try to have either Brody or Ezra as one pitcher and Miles or Ethan as the next and then one new or less experienced pitcher." |
| 💡 | **Primary positions on roster** — Capture each player's primary fielding role(s) so optimal-fielding can suggest assignments. Fields: `roster[i].positions = ['C','1B',...]`. Catchers (Hunter/Ezra/Miles + Jacob developing). 1B (Brody, fallback). Optimal-fielding-by-inning view feeds the Field tab pre-plan grid. | Manager v88 commentary: "Hunter, Ezra, and Miles are our catchers but Jacob is coming along. Brody can play first..." |
| 💡 | **Wildcard / inconsistency flag** — Some kids flash high ability but are unreliable (Julian, Jacob). Capture as `roster[i].wildcard=true`. Optimal-lineup avoids putting wildcards in slot 1-4 (high-leverage); fielding planner avoids high-pressure positions when game is close. | Manager v88: "Julian and Jacob are wildcards in the field. They flash high ability but can be bad." |
| 💡 | **Optimal fielding by inning, weighted by pitcher** — When Ethan or Miles is pitching (strike-throwers, hittable), put best fielders (Ezra, Ethan-when-not-pitching) in the infield. When power pitchers are in (Brody/Ezra), outfield can be looser. Drives the Field tab pre-plan grid: surfaces "suggested" fielding for each inning given the planned pitcher. | Manager v88: "When Ethan and Miles pitch we need our best fielders in the infield. They throw strikes but are hitable and let up a lot of infield grounders." Plus 1B/2B-area-hits insight: "We seem to give up a decent amount of hits to the 1B/2B area." |
