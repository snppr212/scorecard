...
| ✅ **v96 SHIPPED** | **`pscored` schema: store `bi` + `team` + `name`** — Renderer now resolves `bi` to the current lineup name on every render so opp-lineup renames propagate to historical AB sub-lines. Old string entries continue rendering as-is (backward compatible — no data migration). | finAB pushes objects via logRun; plays-log renderer handles both shapes. |
...
| ✅ **Verified 5/6** | **Pinch-run swap-back display verification** — User reported in e8 that only one pinch-run line was visible in plays. Both records ARE in Firestore (T3 #25 Bodhi-for-Malcolm + T3 #26 Malcolm-for-Bodhi after fix). | User confirmed both render correctly. Closed. |
| 🔥 **Next** | **Auto-detect run cap (6-run rule)** — When a half-inning hits 6 runs, prompt "End half-inning? (run-limit cap)". Manager currently has to manually advance. | Came up in T2 + T3 of e9 where both halves hit 6 and ended via cap. **Promoted 5/3:** manager confirmed this is one of the top three priorities to ship next. |
| ✅ **v90 SHIPPED** | **Force-out RBI awareness** — Contextual rule hint under the RBI buttons in the play-log modal, swapping based on result code. Practical interpretation per coach: any time a kid causes a run to score, give them the RBI; only DP/GIDP excludes (and that's an opt-in result the coach taps). The technical LL 9.04 "run scored solely due to fielding error" exception is **NOT** surfaced live — too hard to track in real-time. v96 adds a post-game review for that exception (off by default, toggle-gated). |
| 💡 _someday_ | **Pitch-quality + swing-quality tracking** — Coach 5/6 design: split each pitch into one of: ball, called strike, swing-strike good-pitch, swing-strike bad-pitch, foul good-pitch, foul bad-pitch, and BIP good-pitch / BIP bad-pitch. Discipline metric becomes (taken-balls + swung-good-pitches) ÷ total pitches; passive-walk discount becomes precise (low swing-rate kid who walks a lot is contributing less than a disciplined kid with a similar OBP). Coach concern: "any swing to have a secondary good pitch/bad pitch entry... I would love that, just not sure I can get that all entered in time." Park until live data-entry overhead can be reduced (voice input, parent-helper scorebook, OCR import). | Better than the original "split S into swing/called" design because it captures DECISION QUALITY, not just contact result. Folds back into the lineup formula by replacing OBP weight with a swing-rate-aware OBP. |
| ✅ **v96 SHIPPED** | **Error-attributed RBI post-game review (toggle-gated)** — Setting `A.errorRbiReviewEnabled` (off by default; toggle in Roster modal). When ON, the End Game modal surfaces a "Review N candidates" prompt for any AB where a fielding error and a run-scoring event happened on the same play. Tap → opens review modal listing each candidate with batter / result / errors / current RBI; tap any RBI button to adjust down per LL 9.04. No mid-game friction. | User wanted: "build it and turn it off would be great." Done. Toggle stays off until they explicitly opt in for a calmer post-game review. |
| 🔥 **Next** | **Opp pitcher pitch-count display** — We track opp pitcher totals via `ab.pitcher` attribution (e9 Greg/Paxton/Gold), but there's no clear box-score view showing them per-pitcher per-game. | Side-tab on the Pitchers screen, similar to ours. **Promoted 5/3.** |
| ✅ **v86-v88 SHIPPED** | **Manager intangibles ratings → optimal lineup** — Per-player rating modal (🌟 button on roster rows) with 7 categories: Hitting/Throwing/Fielding/Pitching/Confidence/Discipline/Clutch. 0-5 scale, decimals OK, nullable. Versioned seed migrations (v1 v86, v2 v87, v3 v88) populate from coach-pool + manager assessments without overwriting edits. Optimal-lineup composite blends stats with ratings: `0.3·hittingNorm + 0.1·confNorm + 0.1·discNorm + 0.1·clutchNorm`. | All 12 Mussels seeded; intangibles (Conf/Disc/Clutch) blank for manager to fill in over time. |
| 💡 _deprioritized 5/3_ | **Pitcher rotation tiers** — Manager pairs pitchers in tiers (Brody/Ezra are tier-1; Miles/Ethan are tier-2; everyone else is "developing"). Capture as `roster[i].pitcherTier` (1/2/3/null). Pre-game pitcher-planning view suggests "tier-1 + tier-2 + developing" rotations and warns when one is missing. | Manager feedback v88: "We try to have either Brody or Ezra as one pitcher and Miles or Ethan as the next and then one new or less experienced pitcher." Manager 5/3: "not sure we need any of those." |
| 💡 _deprioritized 5/3_ | **Primary positions on roster** — Capture each player's primary fielding role(s) so optimal-fielding can suggest assignments. Fields: `roster[i].positions = ['C','1B',...]`. Catchers (Hunter/Ezra/Miles + Jacob developing). 1B (Brody, fallback). Optimal-fielding-by-inning view feeds the Field tab pre-plan grid. | Manager v88 commentary: "Hunter, Ezra, and Miles are our catchers but Jacob is coming along. Brody can play first..." Manager 5/3: "not sure we need any of those." |
| 💡 _deprioritized 5/3_ | **Wildcard / inconsistency flag** — Some kids flash high ability but are unreliable (Julian, Jacob). Capture as `roster[i].wildcard=true`. Optimal-lineup avoids putting wildcards in slot 1-4 (high-leverage); fielding planner avoids high-pressure positions when game is close. | Manager v88: "Julian and Jacob are wildcards in the field. They flash high ability but can be bad." Manager 5/3: "not sure we need any of those." |
| 💡 _deprioritized 5/3_ | **Optimal fielding by inning, weighted by pitcher** — When Ethan or Miles is pitching (strike-throwers, hittable), put best fielders (Ezra, Ethan-when-not-pitching) in the infield. When power pitchers are in (Brody/Ezra), outfield can be looser. Drives the Field tab pre-plan grid: surfaces "suggested" fielding for each inning given the planned pitcher. | Manager v88: "When Ethan and Miles pitch we need our best fielders in the infield. They throw strikes but are hitable and let up a lot of infield grounders." Plus 1B/2B-area-hits insight: "We seem to give up a decent amount of hits to the 1B/2B area." Manager 5/3: "not sure we need any of those." |

## Status Key
- ✅ Done
- 🔄 In Progress / Needs Verification
- 🔥 **Next** — manager-confirmed top priority, ship next
