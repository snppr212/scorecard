...
| 💡 | **Manager intangibles ratings → optimal lineup** — The v85 optimal-lineup formula is stats-only (OBP/SLG/BIP/K). Real LL lineup decisions also weigh confidence at the plate, swing ability, plate discipline, situational toughness — things the box score can't see. Add a per-player rating screen (e.g. 1-5 stars on Confidence / Swing / Discipline / Clutch / Speed) and let the optimal-lineup formula combine those scaled ratings with the stat-based composite. Manager updates the ratings between games; algorithm re-ranks accordingly. | User feedback after v85: "Ben leading off is actually not bad because he is confident — you can ask me to rate them on confidence, swing ability, discipline, or other intangibles." Schema: `A.roster[i].ratings = {confidence, swing, discipline, clutch, speed}`. Modal: per-player slider/star UI. Formula: `composite + 0.X·avg_rating` or weighted per category. |
